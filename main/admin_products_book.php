...
if(isset($_POST['add_product'])){
   ...
   if(mysqli_num_rows($select_product_name) > 0){
      ...
      #Insert here
      $select_author=mysqli_query($conn,"SELECT `Fullname` FROM `author` WHERE Fullname='$a_name'") or die ('query failed');

      if(mysqli_num_rows($select_author)>0) {
         $res = mysqli_query($conn,"SELECT `Author_ID` FROM `author` WHERE Fullname='$a_name'") or die ('query failed');
         $result = mysqli_fetch_array($res);
         $AUTHOR_ID = $result['Author_ID'];
      }

      else {
         $add_author=mysqli_query($conn,"INSERT INTO `author` (Fullname) VALUES ('$a_name')") or die('query author failed');   
...
if(isset($_GET['delete'])){
   ...
   header('location:admin_products_book.php');
}
...
<script>
   ...
   window.location.href = 'admin_products_book.php';
}
</script>
