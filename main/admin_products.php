...
if(isset($_GET['delete'])){
   ...
   $delete_image_query = mysqli_query($conn, "SELECT Thumbnail FROM `product` WHERE Product_ID = '$delete_id'") or die('query faileddd');
   $fetch_delete_image = mysqli_fetch_assoc($delete_image_query);
   unlink('uploaded_img/'.$fetch_delete_image['Thumbnail']);
   mysqli_query($conn, "UPDATE `product` SET Deleted = 1 WHERE Product_ID = '$delete_id'") or die('query failedd');
...
<section class="show-products">
   ...
   <div class="box-container">
      ...
      <?php
         $select_products = mysqli_query($conn, "SELECT * FROM `product` where Deleted=0") or die('query failed');
